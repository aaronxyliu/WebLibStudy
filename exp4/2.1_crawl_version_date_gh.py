...
def crawl_tag_date():
    ...
    for i, entry in enumerate(libs):
        ...
        if db2.entry_count(libname, condition="`date` IS NULL AND `estimate date` IS NULL") == 0:
        ...
        # Start date crawling
        res = db2.select_all(libname, ["version"], condition="`date` IS NULL AND `estimate date` IS NULL")
        matched_num = 0
        for entry2 in res:
