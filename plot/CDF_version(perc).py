...
# Load the data
df = pd.read_csv('data/version_hits_distribution(perc).csv')
...
for col in df.columns:
    ...
    plt.plot(x_vals, cdf_vals, label=str(col) + f' (avg: {average * 100:.1f}% versions)')

# Custom ticks
# x_ticks = np.linspace(df.min().min(), df.max().max(), 11)  # 15 evenly spaced x-axis ticks
y_ticks = np.linspace(0, 1, 11)  # 21 evenly spaced y-axis ticks (0.0 to 1.0)
