...
def is_version_in_range(version_str, range_str):
    ...
    Args:
        version_str (str): The version to check (e.g., "4.5.2", "4.0.28-csp")
        range_str (str): The version range (e.g., ">=4.0.0 <4.10.2", "=3.3.8", "*")
    ...
    try:
        ...
        # Special handling for version strings with custom suffixes
        try:
            ver = version.parse(version_str)
        except version.InvalidVersion:
            # For versions with custom suffixes, try parsing just the numeric part
            base_version = re.split(r'[-+]', version_str)[0]
            try:
                ver = version.parse(base_version)
            except version.InvalidVersion:
                raise ValueError(f"Invalid version format: {version_str}")
            
            # Check if the base version would satisfy the specifiers
            # Only return True if ALL specifiers are satisfied by the base version
            return all(ver in s for s in spec)
        
        # Standard case - version parsed successfully
        return spec.contains(ver)
        
    except (version.InvalidVersion, ValueError) as e:
        raise ValueError(f"Invalid version or range: {e}")
...
if __name__ == '__main__':
    db.add_column(VUL_TABLE, '# hits', 'INT')   # One year hits of vulnerable versions
    ...
    for entry in res:
        ...
        db.update(VUL_TABLE, data={'# hits': hits}, condition='synk=%s', condition_values=(synk_link,))
        logger.outdent()
